classdef Swarm < handle
    ...
    properties
        dyn_radii %Boolean value
        zone_overlaps %integer value
        risk_level %cumulative counter
        distance_level %counter (in %) to closest hotpost center
        drones % a vector of Drone objects
        nb_agents % size of the above vector
        zones % a vector of zones
        nb_zones % size of the above vector
        equivalent_drone % for path planner, drone at the barycenter ...
                         % of the swarm for command computations
        algorithm SwarmAlgorithm
        collisions_history
        battery_level %counter (in %) of the battery level
    end
    ...
    methods
        ...
        function self = Swarm()
            ...
            self.risk_level = 0;
            self.zone_overlaps = 0;
            self.dyn_radii = true;
            self.battery_level = 100;
        end
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function add_zone(self, radius)
            self.nb_zones = self.nb_zones + 1;
            self.zones = [self.zones; radius];     
        end
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function radius = get_zone(self, i)
            radius = self.zones(i,1);
        end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function set_zone_overlaps(self, value)

            self.zone_overlaps = value;

        end
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function Value = get_zone_overlaps(self)

            Value = self.zone_overlaps;

        end

        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function Value = get_battery_level(self)
            total=0;
            for i = 1:self.nb_agents
                self.drones(i).update_battery();
                fprintf('battery level 1 %.1f\n',self.drones(i).V);
                fprintf('battery level 1 %.1f\n',self.drones(i).Q);
                total=total+self.drones(i).Q;
            end
            self.battery_level = total;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function level = get_risk_level(self)
            level = self.risk_level;

        end        
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function level = get_nb_agents(self)
            level = self.nb_agents;

        end  

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function level = get_distance_level(self)
            level = self.distance_level;
        ...
        function [vel_commands, collisions] = update_command(self, p_swarm, r_coll, dt)
            ...
            elseif self.algorithm == "ccece_curvilinear"% ccece-paper swarming with curvilinear algorithm
                [vel_commands, collisions] = self.compute_vel_ccece_curvilinear(p_swarm, r_coll, dt);
            elseif self.algorithm == "ccece_rectilinear"% ccece-paper swarming with rectilinear algorithm
                [vel_commands, collisions] = self.compute_vel_ccece_rectilinear(p_swarm, r_coll, dt);
            elseif self.algorithm == "ccece_olfati_saber"% ccece-paper swarming with collision avoidance algorithm
                [vel_commands, collisions] = self.compute_vel_ccece_olfati_saber(p_swarm, r_coll, dt);
            end
