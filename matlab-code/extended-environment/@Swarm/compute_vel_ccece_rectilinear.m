function [V_command, collisions] = compute_vel_ccece_rectilinear(self, p_swarm, r_agent, dt)
    % ccece-paper RECTILINEAR SWARMING ALGORITHM
    % This is an implementation of the RECTILINEAR SWARMING ALGORITHM. It
    % assumes agents want to minimize their contact with risky zones  
    %
    % Ref:
    % Inputs:
    ...
    for agent = 1:nb_agents
        ...
        %% JGA (not affected by zones)
        %% NO NEED FOR MODIFICATIONS HERE
        ...
        % Compute cylindric effect, i.e., ccece zones
        %% ccece algorithm starts here
        if p_swarm.is_active_cyl
            ...
            for obs = 1:p_swarm.n_cyl
                ...
                %% fprintf('obstacle %d: center==[%.1f,%.1f];',obs, c_obs(1), c_obs(2));                
                
                %% Get dynamic zone radii instead of static radii
                if length(radii)>0
                    %%fprintf('radius_1 is %.1f ;',r_obs);
                    %%fprintf('radius_2 is %.1f \n',radii(obs));
                    r_obs=radii(obs);
                end               
                ...
                if (dist_ab < p_swarm.r0)
                    ...
                    if enableRepulsion % enable/disable in variable disableRepulsion
