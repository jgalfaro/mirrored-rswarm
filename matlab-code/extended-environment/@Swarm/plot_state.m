function plot_state(self, time, end_time, iter, period, ...
    ...
    if mod(time, period) == 0
        ...
        v_module = 1 - sqrt(sum((vel.^2), 1));
        v_mean = 1 - mean(v_module);
        v_sd = 1 - std(v_module);

        if ~isempty(collisions)
            n_agent_collisions = collisions(1);%counter
            n_obs_collisions = collisions(2);%counter
            min_dist_obs = collisions(3);%20 to 0
            self.distance_level=min_dist_obs;
        end
        
        %%risk level counter (initially 37.5) 
        if min_dist_obs < 10 
            self.risk_level = self.risk_level + (0.1 * (10-min_dist_obs));
            %disp(['risk level==' string(self.risk_level)]);
        end
        ...
        if isempty(fig_handle)
            ...
            width = 450;%960
            height = 450;%973
            fig_handle = figure('Name', 'Simulation Results', ...
                'NumberTitle', 'off', 'position', [x0, y0, width, height]); clf
            %tg = uitabgroup; % tabgroup

            %tabs(1) = uitab(tg, 'title', 'Simulation Results');
            %axes('Parent', tabs(1));
            ...
            subplot(3, 1, 1);
            hold on
            dist_obs_handle = graph_y(iter, time, min_dist_obs, '{Distance to zone centers [m]}', []);
            obs_thr_handle = graph_thr(iter, time, r_agent, []);
            xlabel('Time [s]', 'fontsize', fontsize);

            subplot(3, 1, 2);
            hold on
            v_msd_handle = graph_y(iter, time, self.risk_level, '{Risk level [%]}', []);
            xlabel('Time [s]', 'fontsize', fontsize);
            
            subplot(3, 1, 3);
            hold on
            b_msd_handle = graph_y(iter, time, 0.034*time, '{Battery consumption [%]}', []);
            xlabel('Time [s]', 'fontsize', fontsize);
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%tabs(2) = uitab(tg, 'title', 'Risk Monitor');
            %%axes('Parent', tabs(2));
            ...
            %%v_msd_handle = graph_mean_sd(iter, time, v_mean, v_sd, 'Risk Level [u^3]', []);
            %%v_msd_handle = graph_y(iter, time, v_mean, 'Risk Level [u^3]', []);
            %%v_msd_handle = graph_y(iter, time, self.risk_level, 'Risk Level [u^3]', []);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            
            %%tabs(3) = uitab(tg, 'title', 'Overlap Monitor');
            ...
            %%ag_col_handle = graph_y(iter, time, n_agent_collisions, '{Collisions between drone}', []);

            %%subplot(1, 1, 1)
            %%hold on
            %obs_col_handle = graph_y(iter, time, n_obs_collisions, 'c_{obstacles}', []);
            %%obs_col_handle = graph_y(iter, time, n_obs_collisions, '{Drone & zones Overlap [counter]}', []);
            ...
            graph_y(iter, time, min_dist_obs, '{Distance to zone centers [ELSE]}', dist_obs_handle);
            graph_thr(iter, time, r_agent, obs_thr_handle);


            %TAB(2) 
            %%graph_mean_sd(iter, time, v_mean, v_sd, 'velocity [m/s]', v_msd_handle);
            %%graph_mean_sd(iter, time, v_mean, v_sd, 'Risk Level [ELSE]', v_msd_handle);
            graph_y(iter, time, self.risk_level, '{Risk level [ELSE]}', v_msd_handle);

            %TAB(3)
            %graph_y(iter, time, n_agent_collisions, 'c_{agents}', ag_col_handle);
            %%graph_y(iter, time, n_obs_collisions, '{Drone & zones Overlap [ELSE]}', obs_col_handle);
            graph_y(iter, time, 0.034*time, '{Battery consumption [ELSE]}', b_msd_handle);
        end
