        ...
        function self = update(self, time, swarm, map)
            ...
            if (time-self.time_of_last_frame) >= self.output_rate - 1e-6
                ...
                if self.plot_initialized == 0 % Init plot
                    ...
                    width = 450;%884
                    height = 450;%973
                    self.figure_handle = figure('Name','Simulation', ...
                        'NumberTitle','off', ...
                    ...
                    if map.ACTIVE_ENVIRONMENT
                        if strcmp(map.building_shape, 'parallelepiped')
                            draw_buildings(map);
                        elseif strcmp(map.building_shape, 'zone')
                            self.figure_handle = draw_zones(self.figure_handle, map, swarm.get_zone_overlaps(), swarm);
                ...
                titleStr=sprintf('\n \n Simulation time: %.1f seconds \n', time);
                titleStr=sprintf('%s Distance to zone centers: %.1f\n', titleStr, swarm.get_distance_level());
                titleStr=sprintf('%s Risk level: %.1f;  ', titleStr, swarm.get_risk_level());
                if time > 1
                    titleStr=sprintf('%s Battery consumption: %.1f\n', titleStr, 0.034*time);
                end
                title(self.figure_handle.CurrentAxes, titleStr);
